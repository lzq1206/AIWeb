...
- GitHub Actions 每 3 小时抓取两组搜索信号，每次更新 20 个项目
- 综合当前星标、近轮次星标增长速度和最近活跃度，优先选择快速升温且已有热度的项目
- 以仓库全名去重，跨批次保留星标快照，避免同一个项目重复出现
...
抓取任务通过 `data/fetch-state.json` 轮换搜索词，每 3 小时只处理两组查询；每组同时覆盖热门排序和最近更新排序，并保留星标快照计算上升速度。
